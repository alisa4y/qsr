...
describe("data item that behaves as array", () => {
  ...
  it("works like an array", done => {
    cy.visit("localhost:3000").then(contentWindow => {
      ...
      sum.contains(30)
      let deletedOnes = list.eval.splice(1, 2, { count: 5 }, { count: 10 })
      ...
      expect(listValue(list.eval)).deep.equal([25, 30, 3, 5, 10, 5, 5])
      cy.get("ul[data-item] > :nth-child(3) span").contains(3)
      cy.get("ul[data-item] > :nth-child(5) span").contains(10)
      sum.contains(83)

      deletedOnes = list.eval.splice(5, 4, { count: 12 }, { count: 11 })
      expect(deletedOnes).deep.equal([{ count: 5 }, { count: 5 }])
      expect(listValue(list.eval)).deep.equal([25, 30, 3, 5, 10, 12, 11])
      sum.contains(40)
      ...
      expect(listValue(list.eval)).deep.equal([25, 30, 3, 5])
      sum.contains(63)

      deletedOnes = list.eval.splice(3, 13)
      expect(deletedOnes).deep.equal([{ count: 5 }])
      expect(listValue(list.eval)).deep.equal([25, 30, 3])
      sum.contains(58)
...
describe("table element ", () => {
  it("works like an array", done => {
    cy.visit("localhost:3000").then(contentWindow => {
      ...
      table.eval = users1
      cy.get("table > tbody > tr:nth-child(4) > td:nth-child(2)").contains(
        "Naisbit"
      ...
      for (let i = 0; i < 6; i++) expect(table.eval[i]).deep.equal(users1[i])
...
describe("attribute data-item-data", () => {
  it("automatically evaluate element with attribute data-item-data to list", done => {
    cy.visit("http://localhost:3000").then(cw => {
      ...
      ;(
        counterForm.querySelector("*[data-item-add]") as HTMLButtonElement
      ).click()
      cy.get("ul[data-item] > div span").contains(11)
      userForm.eval = users2[0]
      ;(userForm.querySelector("*[data-item-add]") as HTMLButtonElement).click()
      cy.get("table[data-item] > tbody > tr > td:nth-child(1)").contains("Dre")
      done()
    })
  })
  it("genuinely detect data-item-edit to change item", done => {
    cy.visit("http://localhost:3000").then(cw => {
      const users = cw.document.querySelector("#users table") as XElement
      users.eval = users2
      ...
      cy.get("#users table > tbody > tr:nth-child(1) td").contains("Druid")
      done()
    })
  })
})
